...
// Tue Oct  1 02:26:56 2019 by ROOT version 6.10/09
// from TTree Ntuple/Expression Ntuple
// found on file: /hdfs/store/user/caillol/SMHTT_2018_17sep_mc/DY3JetsToLL_M-50_TuneCP5_13TeV-madgraphMLM-pythia8_-102X_upgrade2018_realistic_v15-v1/make_ntuples_cfg-E187E67E-2A47-2B44-B437-1235D4E53F33.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   std::vector<float> njetVecUp;
   std::vector<float> njetVecDown;
   ...
   std::vector<float> puppimetVecUp;
   std::vector<float> puppimetVecDown;
   std::vector<float> puppimetphiVecUp;
   std::vector<float> puppimetphiVecDown;
   std::vector<float> mjjVecUp;
   std::vector<float> mjjVecDown;
   std::vector<float> j1ptVecUp;
   std::vector<float> j1ptVecDown;
   std::vector<float> j2ptVecUp;
   std::vector<float> j2ptVecDown;
...
void HTauTauTree_mt::Init(TTree* tree)
{
   ...
   _tree->SetMakeClass(1); // needed especially when compiling
    for(unsigned int i=0; i<uncertNames.size(); i++){
        ...
        puppimetVecUp.push_back(-999.);
        puppimetVecDown.push_back(-999.);
        puppimetphiVecUp.push_back(-999.);
        puppimetphiVecDown.push_back(-999.);
        mjjVecUp.push_back(-999.);
        mjjVecDown.push_back(-999.);
        j1ptVecUp.push_back(-999.);
        j1ptVecDown.push_back(-999.);
        j2ptVecUp.push_back(-999.);
        j2ptVecDown.push_back(-999.);
    }

    //addressing the jec vectors
    for(unsigned int i=0; i<uncertNames.size(); i++){
        _tree->SetBranchAddress(("jetVeto30_Jet"+uncertNames[i]+"Up").c_str(),     &njetVecUp[i]                        );
        _tree->SetBranchAddress(("jetVeto30_Jet"+uncertNames[i]+"Down").c_str(),     &njetVecDown[i]                    );
        _tree->SetBranchAddress(("vbfMass_Jet"+uncertNames[i]+"Up").c_str(),     &vbfVecUp[i]                           );
        _tree->SetBranchAddress(("vbfMass_Jet"+uncertNames[i]+"Down").c_str(),     &vbfVecDown[i]                       );
        _tree->SetBranchAddress(("type1_pfMet_shiftedPt_Jet"+uncertNames[i]+"Up").c_str(),     &metVecUp[i]             );
        _tree->SetBranchAddress(("type1_pfMet_shiftedPt_Jet"+uncertNames[i]+"Down").c_str(),     &metVecDown[i]         );
        _tree->SetBranchAddress(("type1_pfMet_shiftedPhi_Jet"+uncertNames[i]+"Up").c_str(),     &metphiVecUp[i]         );
        _tree->SetBranchAddress(("type1_pfMet_shiftedPhi_Jet"+uncertNames[i]+"Down").c_str(),     &metphiVecDown[i]     );
        _tree->SetBranchAddress(("j1pt_Jet"+uncertNames[i]+"Up").c_str(),     &j1ptVecUp[i]                             );
        _tree->SetBranchAddress(("j1pt_Jet"+uncertNames[i]+"Down").c_str(),     &j1ptVecDown[i]                         );
        _tree->SetBranchAddress(("j2pt_Jet"+uncertNames[i]+"Up").c_str(),     &j2ptVecUp[i]                             );
        _tree->SetBranchAddress(("j2pt_Jet"+uncertNames[i]+"Down").c_str(),     &j2ptVecDown[i]                         );
